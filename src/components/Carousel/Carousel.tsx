    ...
    <div className={`relative h-[30rem] w-full overflow-hidden ${className}`}>
      ...
      {SHOW_LEFT_ARROW && (
        <button
          onClick={moveToPrev}
          className={`
            hidden min-[475px]:block absolute outline-0 text-3xl text-primary bottom-1/2 left-2 
      ...
      {SHOW_RIGHT_ARROW && (
        <button
          onClick={moveToNext}
          className={`
            hidden min-[475px]:block absolute outline-0 text-3xl text-primary bottom-1/2 right-2 
            [transition:_color_300ms_ease-out_transform_300ms_ease-out] z-10
            hover:text-accent hover:scale-[1.2] 
            focus:text-accent focus:scale-[1.2]
        `}
